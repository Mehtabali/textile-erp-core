...
public sealed class InternalUsersController(IInternalUserService internalUserService) : ApiControllerBase
{
    ...
    [SwaggerOperation(
        ...
        Description = "Returns the first page of internal users from dbo.USERS where ROLE = 0, ROLE = 6, or ROLE = 7.")]
    ...
    [SwaggerOperation(
        Summary = "List internal users",
        Description = "Returns paged internal users from dbo.USERS where ROLE = 0, ROLE = 6, or ROLE = 7. Search, filters, sort, and paging are supplied in the request body.")]
    [ProducesResponseType(typeof(InternalUserListResponse), StatusCodes.Status200OK)]
    ...
    [SwaggerOperation(
        ...
        Description = "Returns one internal user by USERID. Only ROLE = 0, ROLE = 6, or ROLE = 7 records are returned.")]
    ...
    [SwaggerOperation(
        ...
        Description = "Creates an internal user in dbo.USERS with ROLE = 0, ROLE = 6, or ROLE = 7. Password is stored in PASSWORDHASH and confirmPassword must match password.")]
