...
[SwaggerTag("Internal user management endpoints. These endpoints only read and write USERS rows with ROLE = 5 or ROLE = 6.")]
public sealed class InternalUsersController(IInternalUserService internalUserService) : ControllerBase
{
    ...
    [SwaggerOperation(
        ...
        Description = "Returns paged internal users from dbo.USERS where ROLE = 5 or ROLE = 6. Search, filters, sort, and paging are supplied in the request body.")]
    ...
    [SwaggerOperation(
        ...
        Description = "Returns one internal user by USERID. Only ROLE = 5 or ROLE = 6 records are returned.")]
    ...
    [SwaggerOperation(
        ...
        Description = "Creates an internal user in dbo.USERS with ROLE = 5 or ROLE = 6. Password is stored in PASSWORDHASH and confirmPassword must match password.")]
