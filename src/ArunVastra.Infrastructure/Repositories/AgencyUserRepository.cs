...
public sealed class AgencyUserRepository : IAgencyUserRepository
{
    private const int AgencyRole = (int)UserRole.Agency;

    private readonly ArunVastraDbContext _dbContext;

    public AgencyUserRepository(ArunVastraDbContext dbContext)
    {
        _dbContext = dbContext;
    }

    public async Task<AgencyUserListResponse> ListAsync(
        AgencyUserListRequest request,
        CancellationToken cancellationToken = default)
    {
        var query = _dbContext.Users
            .AsNoTracking()
            .Where(user => user.Role == AgencyRole);
        ...
        var items = await query
            ...
            .Select(user => new AgencyUserListItemResponse
            {
                UserId = user.Userid,
                Name = user.Firstname,
                Email = user.Email,
        ...
        return new AgencyUserListResponse
        {
            Items = items,
    ...
    public async Task<AgencyUserResponse?> GetByIdAsync(
        int userId,
        CancellationToken cancellationToken = default)
    {
        return await _dbContext.Users
            .AsNoTracking()
            .Where(user => user.Userid == userId && user.Role == AgencyRole)
            .Select(user => new AgencyUserResponse
            {
                UserId = user.Userid,
                Name = user.Firstname,
                Email = user.Email,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return await GetByIdAsync(user.Userid, cancellationToken) ?? ToAgencyUserResponse(user);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        var user = await _dbContext.Users
            .SingleOrDefaultAsync(
                item => item.Userid == userId && item.Role == AgencyRole,
                cancellationToken);
    ...
    private IQueryable<User> ApplyColumnFilters(
        ...
        AgencyUserListRequest request)
    {
        var filters = request.Filters ?? new AgencyUserListFiltersRequest();
    ...
    private static AgencyUserResponse ToAgencyUserResponse(User user)
    {
